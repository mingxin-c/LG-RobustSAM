...
# 🔥 新增：中间特征蒸馏配置
# Apply pixel-wise distillation on intermediate features (mid_1280)
_C.DISTILL.MID_PIXEL_WISE = -1.0
# Apply channel-wise distillation on intermediate features
_C.DISTILL.MID_CHANNEL_WISE = -1.0
# Apply correlation distillation on intermediate features
_C.DISTILL.MID_CORRELATION = -1.0
...
# 严格对齐 RobustSAM：仅解码器侧的一致性（不包含中间特征）
# L_MFC: Mask Feature Consistency Loss（decoder feat）
_C.DISTILL.ROBUST_MFC_DECODER = -1.0
# L_TC: Token Consistency Loss
_C.DISTILL.ROBUST_TC_QUERY = -1.0
_C.DISTILL.ROBUST_TC_ATTN = -1.0
# L_Seg: Segmentation Consistency Loss
_C.DISTILL.ROBUST_SEG_DICE = -1.0
_C.DISTILL.ROBUST_SEG_FOCAL = -1.0
_C.DISTILL.ROBUST_SEG_BCE = -1.0
...
# --- Added: logits-level segmentation + smoothness (default off) ---
# Overall weight for logits-level seg loss; <=0 disables
_C.DISTILL.LOGITS_SEG_LOSS = -1.0
# BCE and Dice composition inside logits-level seg loss
_C.DISTILL.LOGITS_SEG_BCE = 0.5
_C.DISTILL.LOGITS_SEG_DICE = 0.5
# Smoothness regularization on logits (TV/Laplacian); <=0 disables
_C.DISTILL.LOGITS_TV = 0.0
_C.DISTILL.LOGITS_LAP = 0.0
