...
class SamBatch(Sam):
    ...
    def forward(self, mode, **kwargs):
        if mode == 'image_encoder':
            ...
            # Map EdgeSAM-style arg 'num_multimask_outputs' -> our 'multimask_output' boolean
            if 'num_multimask_outputs' in kwargs and 'multimask_output' not in kwargs:
                nmo = kwargs.pop('num_multimask_outputs')
                try:
                    kwargs['multimask_output'] = bool(nmo) and (nmo != 1)
                except Exception:
