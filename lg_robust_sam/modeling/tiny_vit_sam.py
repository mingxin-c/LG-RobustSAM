...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        # 🔥 双输出改造：添加中间层投影到1280通道（对齐教师中间层）
        # Stage 1后提取早期特征，已经是64x64，直接投影到1280通道
        self.mid_proj = nn.Conv2d(
            embed_dims[2],   # Stage 1输出通道数（经过PatchMerging后是embed_dims[2]=384）
            1280,            # 输出：对齐教师中间层
            kernel_size=1,
            bias=False,
        )

        # 保留原有neck：最终层投影到256通道（对齐教师最终层）
    ...
    def forward_features(self, x):
        ...
        x = self.layers[0](x)  # Stage 0: ConvLayer
        x = self.layers[1](x)  # Stage 1: BasicLayer (包含PatchMerging)

        # 🔥 在Stage 1后提取早期特征（对应教师第8层/32层的相对位置）
        # x: [B, N, C] 序列格式，其中N=64*64，C=embed_dims[2]=384
        # 需要还原成BCHW格式进行卷积
        B, N, C = x.size()
        H = W = int(N ** 0.5)  # 64x64
        mid_x_bchw = x.view(B, H, W, C).permute(0, 3, 1, 2)  # [B, 384, 64, 64]

        # 通过mid_proj投影：[B, 384, 64, 64] -> [B, 1280, 64, 64]
        mid_1280_bchw = self.mid_proj(mid_x_bchw)
        ...
        # 最终层特征处理：[B, N, C_final] -> [B, 256, 64, 64]
        B, N, C_final = x.size()
        H = W = int(N ** 0.5)  # 64x64
        x_final_bchw = x.view(B, H, W, C_final).permute(0, 3, 1, 2)
        final_256 = self.neck(x_final_bchw)  # [B, 256, 64, 64] (BCHW)

        # 学生中间特征改为与教师一致的 BHWC，便于直接喂给解码器中AMFG入口
        mid_1280_bhwc = mid_1280_bchw.permute(0, 2, 3, 1)  # [B, 64, 64, 1280]

        return final_256, mid_1280_bhwc

    def forward(self, x):
        final_256, mid_1280 = self.forward_features(x)
        # 🔥 双输出改造：返回两个对齐的特征
        # final_256: [B, 256, 64, 64] - 用于蒸馏教师最终层和解码器
        # mid_1280: [B, 1280, 64, 64] - 用于蒸馏教师中间层和AOTG/AMFG
        return final_256, mid_1280
