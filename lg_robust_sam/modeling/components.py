...
class TokenBlock(nn.Module):
    """
    Batch-safe token block that treats each token vector (transformer_dim) as
    input_dim groups, each of size mlp_dim. Supports inputs of shape [N, transformer_dim]
    where N can be batch size or sum(num_prompts).
    Produces output with the same leading dims, i.e., [N, transformer_dim].
    """
    def __init__(self, input_dim, mlp_dim):
        super(TokenBlock, self).__init__()
        self.input_dim = input_dim  # e.g., num_mask_tokens
        self.mlp_dim = mlp_dim      # e.g., transformer_dim // num_mask_tokens

        # Per-group MLP applied on the last dim (size=mlp_dim)
        ...
        # Normalization across the group feature dim
        self.norm = nn.LayerNorm(self.mlp_dim)

    def forward(self, x, mlp=True):
        """
        Args:
            x: Tensor of shape [N, input_dim * mlp_dim] (e.g., [N, 256])
            mlp: whether to apply MLP
        Returns:
            Tensor of shape [N, input_dim * mlp_dim]
        """
        N, D = x.shape
        assert D == self.input_dim * self.mlp_dim, \
            f"TokenBlock expects last dim {self.input_dim * self.mlp_dim}, got {D}"

        # reshape to [N, input_dim, mlp_dim]
        x = x.view(N, self.input_dim, self.mlp_dim)
        # normalize per-group feature dim
        x = self.norm(x)

        if mlp:
            # apply MLP to each group independently
            x = self.group_mlp(x)

        # flatten back to [N, input_dim * mlp_dim]
        x = x.reshape(N, self.input_dim * self.mlp_dim)
        return x
